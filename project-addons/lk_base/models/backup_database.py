...
class DbBackup(models.Model):
    ...
    host = fields.Char('Address', required=True, default='localhost')
    ...
    sftp_user = fields.Char('SFTP Server Account',
                            help='The username where the SFTP connection should be made with. This is the user on the '
                                 'external server.')
    sftp_password = fields.Char('SFTP Server Password',
    ...
    days_to_keep_sftp = fields.Integer('Automatically Remove Backup Files In SFTP (day)',
    ...
    def test_sftp_connection(self, context=None):
        ...
        for rec in self:
            ...
            try:
                ...
                sftp = s.open_sftp()
                message_title = _("Connect SFTP successfully!")
            except Exception as e:
                _logger.critical('There is error when connecting toftp: ' + str(e))
                error += str(e)
                has_failed = True
                message_title = _("Connect SFTP unsuccessfully!")
                if len(rec.sftp_host) < 8:
                    message_content += _("\nInvalid IP address.\n")
                message_content += _("Message:\n")
            finally:
                if s:
                    s.close()
        if has_failed:
            ...
            if rec.sftp_write is True:
                try:
                    ...
                    try:
                        ...
                        pass
                    _logger.error(
                        'Exception! We couldn\'t back up to the FTP server. Here is what we got back instead: %s' % str(
                            e))
                    ...
                    if rec.send_mail_sftp_fail:
                        try:
                            ...
                            message = _("Dear,\n\nThe backup for the server %s (IP: %s"") failed. Please check the "
                                        "following details:\n\nIP address SFTP server: %s "
                                        "\nUsername: %s\n\nError details: %s\n\n"
                                        "With kind regards") % \
                                      (rec.host, rec.sftp_host, rec.sftp_host, rec.sftp_user, tools.ustr(e))
                            ...
                            msg = ir_mail_server.build_email(response_mail, [rec.email_to_notify],
                                                             _("Backup from %s (%s) failed") % (rec.host, rec.sftp_host),
                                                             message)
    ...
    def _take_dump(self, db_name, stream, model, backup_format='zip'):
        ...
        cron_user_id = self.env.ref('lk_base.backup_scheduler').user_id.id
